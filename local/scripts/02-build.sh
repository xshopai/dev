...
# =============================================================================
# Each service writes its own log to dev/logs/<service>.log.
...
for svc in "${NODE_SERVICES[@]}"; do
  build_service "$svc" "$WORKSPACE_ROOT/$svc" "npm ci --prefer-offline --no-audit 2>&1" &
done
...
build_service "product-service" "$WORKSPACE_ROOT/product-service" \
  "python3 -m venv venv && venv/bin/pip install -q -r requirements.txt 2>&1 || python -m venv venv && venv/Scripts/pip install -q -r requirements.txt 2>&1" &

build_service "inventory-service" "$WORKSPACE_ROOT/inventory-service" \
  "python3 -m venv venv && venv/bin/pip install -q -r requirements.txt 2>&1 || python -m venv venv && venv/Scripts/pip install -q -r requirements.txt 2>&1" &
...
build_service "order-service" "$WORKSPACE_ROOT/order-service" \
  "dotnet restore OrderService.sln && dotnet build OrderService.sln --no-restore -c Release 2>&1" &

build_service "payment-service" "$WORKSPACE_ROOT/payment-service" \
...
build_service "order-processor-service" "$WORKSPACE_ROOT/order-processor-service" \
